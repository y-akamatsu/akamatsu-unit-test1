...
describe('Personオブジェクトに実装されているメソッドのテスト', () => {
  ...
  it('私の名前は赤松由崇です。', () => {
    assert.equal(person1.getFullName, '私の名前は赤松由崇です。');
  });
  ...
  it('赤松さんのBMIは22です。', () => {
    assert.equal(person1.calculationBMI(), 22);
  });

  const person2 = new Person('孫', '悟空', 737, '男', 1.75, 62);
  it('私の名前は孫悟空です。', () => {
    assert.equal(person2.getFullName, '私の名前は孫悟空です。');
  });
  ...
  it('孫さんのBMIは20です。', () => {
    assert.equal(person2.calculationBMI(), 20);
  });

  const person3 = new Person('山田', '花子', 20, '女', 1.68, 54);
  it('私の名前は山田花子です。', () => {
    assert.equal(person3.getFullName, '私の名前は山田花子です。');
  });
  ...
  it('山田さんのBMIは19です。', () => {
    assert.equal(person3.calculationBMI(), 19);
  });
});
